...
module.exports = {
    ...
    populates: {
          ...
          return this.Promise.all(
            articles.map(article =>
              ctx
                ...
                .then(res => (article.favorited = res)),
            ),
          );
        else {
      ...
      favoritesCount(ids, articles, rule, ctx) {
        return this.Promise.all(
          articles.map(article =>
            ctx
              .call('favorites.count', { article: article._id.toString() })
              .then(count => (article.favoritesCount = count)),
          ),
        );
      },
    },
  ...
  actions: {
        ...
        return this.Promise.resolve()
          .then(() => {
            if (ctx.params.author) {
              return ctx
                .call('users.find', { query: { username: ctx.params.author } })
                .then(users => {
                  if (users.length == 0)
                    return this.Promise.reject(
                      new MoleculerClientError('Author not found'),
                    );
            ...
            if (ctx.params.favorited) {
              return ctx
                .call('users.find', {
                  query: { username: ctx.params.favorited },
                })
                .then(users => {
                  if (users.length == 0)
                    return this.Promise.reject(
    ...
    remove: {
      ...
      handler(ctx) {
        return this.findBySlug(ctx.params.id).then(entity => {
          if (!entity)
            return this.Promise.reject(
              new MoleculerClientError('Article not found!', 404),
            );
          ...
          return this.adapter
            .removeById(entity._id)
            .then(() =>
              ctx.call('favorites.removeByArticle', { article: entity._id }),
            )
            .then(json =>
    ...
    tags: {
      ...
      handler(ctx) {
        return this.Promise.resolve()
          .then(() =>
            this.adapter.find({ fields: ['tagList'], sort: ['createdAt'] }),
          )
          .then(list => {
      ...
      handler(ctx) {
        return this.Promise.resolve(ctx.params.slug)
          ...
          .then(article => {
            ...
            return ctx.call('comments.create', {
              article: article._id.toString(),
      ...
      handler(ctx) {
        return this.Promise.resolve(ctx.params.slug)
          ...
          .then(article => {
            ...
            return ctx.call('comments.update', {
      ...
      handler(ctx) {
        return this.Promise.resolve(ctx.params.slug)
          ...
          .then(article => {
            if (!article)
              return this.Promise.reject(
                new MoleculerClientError('Article not found'),
              );
  ...
  methods: {
    ...
    transformResult(ctx, entities, user) {
      if (Array.isArray(entities)) {
        return this.Promise.map(entities, item =>
          this.transformEntity(ctx, item, user),
        ).then(articles => ({ articles }));
      } else {
        return this.transformEntity(ctx, entities, user).then(article => ({
          article,
        }));
      }
    },
