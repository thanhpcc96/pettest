...
module.exports = {
  ...
  settings: {
    ...
    /** Public fields */
    fields: ['_id', 'username', 'email', 'type'], //'photo_url', 'viewed','likes'],
    ...
    entityValidator: {
      ...
      viewed: { type: 'array', optional: true },
      likes: { type: 'array', optional: true },
    },

    /** for populate with other collection */
    // populates: {
    //   ''
    // }
  },
      ...
      handler(ctx) {
        ...
        return this.validateEntity(entity)
          ...
          .then(() => {
            ...
            entity.type = entity.type || 'Normal';
            entity.photo_url = entity.photo_url || null;
            ...
            return this.adapter
              ...
              .then(user =>  this.transformAuthJson(ctx,user))
              .then(json =>
  ...
  methods: {
    ...
    transformProfile(ctx, user, loggedInUser) {
      //user.image = user.image || "https://www.gravatar.com/avatar/" + crypto.createHash("md5").update(user.email).digest("hex") + "?d=robohash";
      user.photo_url = _.isNull(user.photo_url)
    ...
    /**
     * Tranform returned user to AuthJson with prop:  _id, token
     * @param {Context} ctx
     * @param {Object} user
     * @param {Object?} loggedInUser
     */
    transformAuthJson(ctx, user, loggedInUser) {
      return {
        _id: user._id,
        token: user.token,
      };
